...
def Model():
    ...
    r = Encoder2(dff=2, num_heads=1, num_layers=6)(input_right)
    r = tf.transpose(r, (0, 2, 1))
    r = tf.nn.avg_pool(r, ksize=[8], strides=[1], padding='VALID')
    # r = Bidirectional(LSTM(128, return_sequences=False))(input_right)
    output_right = BatchNormalization()(r)
    output_right = Flatten()(output_right)
    output_right = tf.keras.layers.Reshape(-1, 1)(output_right)
